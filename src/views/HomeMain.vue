...
const queryBannerInfo = async () => {
  try {
    ...
    bannerImages.value = data?.banner?.flatMap((item: any) => {
      return baseUrl.replace(/\/$/, '') + item.path
    })
  } finally {
  }
}
  ...
  <div class="home-page">
    ...
    <div class="home-content">
      ...
      <div class="notice-bar-wrapper">
        <van-notice-bar :text="t('home.notice')" scrollable background="transparent" color="rgb(108, 117, 125)" />
      </div>

      <!-- 快捷操作 -->
      <div class="quick-actions">
        <a href="#/language" class="action-item">
          <img class="action-icon-img" src="../assets/images/lang.png" alt="Language" />
          <div class="action-text">{{ t('home.switchLanguage') }}</div>
        </a>
        <a href="#/about" class="action-item">
          <img class="action-icon-img" src="../assets/images/user/company.png" alt="About" />
          <div class="action-text">{{ t('home.aboutUs') }}</div>
        </a>
        <a href="#/help" class="action-item">
          <img class="action-icon-img" src="../assets/images/duihua.png" alt="Help" />
          <div class="action-text">{{ t('home.helpCenter') }}</div>
        </a>
      </div>
      ...
      <div class="section product-section">
        <div class="section-title">
          <span class="text">—— {{ t('home.productRecommend') }} ——</span>
        </div>

        <div class="product-scroll">
          <div v-for="(product, index) in products" :key="index" class="product-card" @click="goToTrade(product.id)">
            <div class="product-name">{{ product.title }}</div>
            <div class="product-price">{{ product.Price }}</div>
            <img :src="product.trendImg" alt="Chart" class="product-chart-img" />
            <div class="product-price-bottom">{{ product.Price }}</div>
            <div
              :class="[
                'product-change',
                product.trend === 'up' ? 'trend-up' : product.trend === 'down' ? 'trend-down' : 'trend-neutral',
              ]"
            >
              <img :src="product.arrow" alt="Arrow" class="arrow-icon" />
              {{ product.change }}
            </div>
          </div>
        </div>
      ...
      <div class="section market-section">
        <div class="section-title">
          <span class="text">—— {{ t('home.market') }} ——</span>
        </div>
